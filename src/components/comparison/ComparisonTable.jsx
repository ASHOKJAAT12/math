...
const ComparisonTable = ({ results }) => {
    ...
    const hasExactRoot = validMethods.some(m => m.absoluteError !== null);
            ...
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800 border-x border-slate-200 dark:border-slate-800">
                <thead className="bg-slate-100/50 dark:bg-slate-800/50">
                    <tr>
                        ...
                        <th scope="col" className="px-4 py-3 text-right text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Approx Root</th>
                        <th scope="col" className="px-4 py-3 text-right text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Iterations</th>
                    ...
                    {validMethods.map((method, idx) => (
                        <tr key={method.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-right text-indigo-700 dark:text-indigo-400 font-mono font-semibold">
                                {method.root !== null ? formatNumber(method.root) : '---'}
                            </td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-right text-slate-600 dark:text-slate-400">
                                {method.iterations}
                            </td>
